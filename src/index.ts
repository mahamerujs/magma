...
export default class Magma extends MahameruPlugin<MagmaOptions> {
    ...
    constructor(options: Partial<MagmaOptions>) {
        ...
        this.httpServer.on('error', (error: unknown) => {
            if (error instanceof Error && 'code' in error && error.code === 'EADDRINUSE') {
                ...
                throw new MagmaError(`Port ${newError.port} is already in use`, {
                    code: newError.code,
    ...
    protected async handleRequest(request: IncomingMessage, response: ServerResponse<IncomingMessage> & { req: IncomingMessage; }) {
        ...
        try {
            ...
            if (
                typeof this._options.allowedIps !== 'undefined' &&
                magmaRequest.ipAddress
            ) {
                if (!this._options.allowedIps.includes(magmaRequest.ipAddress))
                    return this.sendResponse(response, new MagmaResponse(JSON.stringify({ error: 'Forbidden' }), { status: 403 }));
            }

            if (typeof this._options.allowedHosts !== 'undefined' && Array.isArray(this._options.allowedHosts))
            ...
            if (this._options.trailingSlash === false && rawReqUrl.length > 1 && rawReqUrl.endsWith('/')) {
                ...
                return;
            } else if (this._options.trailingSlash === true && !rawReqUrl.endsWith('/') && !rawReqUrl.includes('.')) {
                this.sendResponse(response,
                    new MagmaResponse(
                        JSON.stringify({ message: 'Redirecting to non-trailing slash URL' }),
            ...
            if (rawReqPath !== rawReqUrl) {
                const queryStr = magmaRequest.url?.includes('?') ? '?' + magmaRequest.url.split('?')[1] : '';
                const redirectPath = rawReqUrl + queryStr;

                return this.sendResponse(response, new MagmaResponse(JSON.stringify({ message: 'Redirecting to normalized URL' }), { status: 301, headers: { Location: redirectPath } }));
            }

            const { matchedRoute, matchResult, notFoundResponse } = await this.route.resolveRoute(magmaRequest);
            ...
            if (!matchedRoute || !matchResult) {
                ...
                const magmaResponse = this.normalizeMagmaResponse(rawResponse, 'Middleware must return a MagmaResponse instance.');
    ...
    protected async loadFavicon() {
        ...
        const defaultFaviconPath = join(process.cwd(), 'node_modules', '@mahameru/magma', 'favicon.ico');
    ...
    protected async handleFaviconRequest(request: MagmaRequest, response: DefaultHTTPResponse) {
        ...
        if (middlewareHandler) {
            const middlewareResponse = await middlewareHandler(context, async () => new MagmaResponse(this._favicon, { status: 200 }));
            ...
            return this.sendResponse(response, new MagmaResponse(normalized.body, {
    ...
    protected sendResponse(response: DefaultHTTPResponse, magmaResponse?: MagmaResponse) {
        if (!magmaResponse)
            magmaResponse = new MagmaResponse(undefined, { status: 204 });

        if (
            magmaResponse.headers.get('X-Powered-By') ||
            magmaResponse.headers.get('x-powered-by')
        ) {
        ...
        if (
            magmaResponse.headers.get('X-Message') ||
            magmaResponse.headers.get('x-message')
        ) {
    ...
    protected async runErrorHandler(error: unknown, context: MagmaContext): Promise<MagmaResponse> {
        ...
        if (!this.container.errorHandler)
            return fallbackResponse;

        try {
            const handlerResponse = await this.container.errorHandler(context, error, async () => fallbackResponse)
    ...
    protected createInternalServerErrorResponse(error: unknown): MagmaResponse {
        const serverError = error instanceof MagmaErrorResponse
            ? error
...
export class MagmaGenerator extends Generator<MagmaGeneratorOptions> {
    ...
    public async routeIndexFile(routesIndexFilePath: string) {
        if (!existsSync(routesIndexFilePath))
            return;
...
`;
    ...
    protected async magmaContainer() {
        ...
        if (existsSync(modulesDtsFilePath))
            dependenciesDtsFilePath.push(modulesDtsFilePath);

        if (existsSync(instancesDtsFilePath))
